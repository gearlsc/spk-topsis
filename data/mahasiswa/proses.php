...
if(isset($_SESSION['username']) AND isset($_SESSION['password'])){
	...
	if($loadPage=="mahasiswa" AND $action=="simpanData"){
		  ...
		  $cariData=mysql_query("SELECT * FROM tmahasiswa WHERE username LIKE '% $_POST[txtUsername] %' OR
	...
	window.location=('../../framemhs.php?loadPage=mahasiswa&action=input')
	</script>
	"; 
		  }else{
		  if(empty($addres_file)){
			  			
		$_SQL = "INSERT INTO tmahasiswa (username,password,nmLengkap,
		...
		window.location=('../../framemhs.php?loadPage=mahasiswa')
		</script>
		";
		  }else{
	...
	}elseif($loadPage=="mahasiswa" AND $action=="hapusData"){
		
	$Query=mysql_query("SELECT foto FROM tmahasiswa WHERE kdMahasiswa ='$_GET[id]'");
	$remove= mysql_fetch_array($Query);
	
	if($remove['foto'] !=""){
		mysql_query("DELETE FROM tmahasiswa WHERE kdMahasiswa=$_GET[id]")or die (mysql_error());
	...
	";
		...
		}elseif($loadPage=="mahasiswa" AND $action=="ubahData"){
		  $addres_file = $_FILES['upPhoto']['tmp_name'];
		  $tipe_file   = $_FILES['upPhoto']['type'];
		  $filename    = $_FILES['upPhoto']['name'];
		  $enkrip	   = md5($filename);
		  $filenameenkrip = $enkrip.$filename;
			if(empty($addres_file)){
				$_SQL= "UPDATE tmahasiswa SET username='$_POST[txtUsername]',
				...
				";
				 ...
				 if($tipe_file !="image/jpg" AND $tipe_file != "image/jpeg"){
				  ...
				  <script language='javascript'>
				  window.alert('Upload Gambar Gagal Pastikan File Bertipe JPEG');
				  window.location=('../../framemhs.php?loadPage=mahasiswa&action=edit&id=$_POST[id]')
				  </script>
				  ";
				  }else{
				upPengguna($filenameenkrip);	  
